...
-- col pid for a5;
col sid for 9999;
col serial# for 99999 heading "SER";
col status for a6;
col command for a10;
col usr for a4;
col machine for a10;
...
col sql for a88;
col ELS for 999,999;
select  --'ALTER SYSTEM KILL SESSION '||''''||se.sid||','||serial#||'''' as kill_txt,
--        process pid,
        se.sid,
        serial#,
        status,
        decode(username,'OMPDBA','OMP','OMPDBA_READ','OMPR') usr,
    ...
    --   terminal,
       machine,
       se.sql_hash_value as hash_value,
       substr(sq.sql_text,1,88) "SQL"
--       to_char(se.logon_time, 'yyyy/mm/dd hh24:mi:ss') start_time
       ,last_call_et "ELS_TIME", io.block_gets
from   v$session se,
       v$sess_io io,
       v$sql sq
where  status != 'INACTIVE'
and    username != ' '
and    (username in ('OMPDBA', 'OMPDBA_READ') or se.SCHEMANAME IN ('OMPDBA','OMPDBA_READ'))
and    se.sid = io.sid
